...
class YammerUserDeleteV2
  ...
  def execute()
    ...
    if users.first
      # Deleting the user (by adding the 'delete=true' parameter to deactivation)
      puts "Deactivating the user" if @enable_debug_logging
      result_json = RestClient.delete("https://www.yammer.com/api/v1/users/#{users.first['id']}.json?delete=true",
        :authorization=>"Bearer #{@info_values['access_token']}")
    end
